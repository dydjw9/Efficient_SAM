...
def train(args,model):
    ...
    optimizer = SAM(paras, base_optimizer, rho=args.rho, weight_dropout=args.weight_dropout,adaptive=args.isASAM,nograd_cutoff=args.nograd_cutoff,opt_dropout = args.opt_dropout,temperature=args.temperature)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in dataset.train:
            ...
            optimizer.step()
            predictions,loss,g_fnc = optimizer.returnthings
        ...
        time_per_epoch = used_time if used_time < time_per_epoch else time_per_epoch
    ...
    if args.local_rank in [-1,0]:
        email_text = "time of {} finished, best time is {}".format(args.name,time_per_epoch) 
        import socket 
        send_email(email_text,"time cal"+socket.gethostname())
        log.flush()
